...
```
SHELL=/bin/bash
0 1 * * 3 source /etc/profile.d/rvm.sh && cd /var/www/sleepdata.org && rvm 2.6.0 && rails weekly_reviewer_digest RAILS_ENV=production
```
...
```
SHELL=/bin/bash
0 1 * * * source /etc/profile.d/rvm.sh && cd /var/www/sleepdata.org && rvm 2.6.0 && rails sitemap:refresh RAILS_ENV=production
```
