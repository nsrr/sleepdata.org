...
task weekly_reviewer_digest: :environment do
  ...
  # At 1am once a week, in production mode, for members who have "daily digest"
  # email notification selected.
  User.current.find_each do |user|
    UserMailer.reviewer_digest(user).deliver_now if user.digest_reviews.count.positive?
  end
end
