...
class TopicsControllerTest < ActionController::TestCase
  ...
  test 'should update topic' do
    login(users(:valid))
    patch :update, id: @topic, topic: topic_params.merge(title: 'Updated Topic Title')
    assert_not_nil assigns(:topic)
  ...
  test 'should not update topic with blank title' do
    login(users(:valid))
    patch :update, id: @topic, topic: topic_params.merge(title: '')
  ...
  test 'should not update topic as banned user' do
    login(users(:banned))
    patch :update, id: topics(:banned), topic: topic_params.merge(title: 'Updated Topic Title')
    assert_nil assigns(:topic)
    assert_redirected_to topics_path
  end
  ...
  test 'should sticky a topic as a system admin' do
    login(users(:admin))
    post :admin, id: @topic, topic: { pinned: '1' }
    assert_not_nil assigns(:topic)
