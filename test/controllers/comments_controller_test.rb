...
class CommentsControllerTest < ActionController::TestCase
  ...
  test 'should create comment and not update existing subscription' do
    ...
    assert_difference('Comment.count') do
      post :create, params: {
        topic_id: @topic,
        comment: { description: 'This is my contribution to the discussion.' }
      }
    end
  ...
  test 'should create comment and add subscription' do
    ...
    assert_difference('Comment.count') do
      post :create, params: {
        topic_id: @topic,
        comment: { description: 'With this comment I am subscribing to the discussion.' }
      }
    end
  ...
  test 'should not create comment as anonymous user' do
    assert_difference('Comment.count', 0) do
      post :create, params: {
        topic_id: @topic, comment: { description: 'I am not logged in.' }
      }
    end
  ...
  test 'should not create comment as banned user' do
    ...
    assert_difference('Comment.count', 0) do
      post :create, params: {
        topic_id: @topic,
        comment: { description: 'I am banned from creating comments.' }
      }
    end
  ...
  test 'should get edit' do
    login(users(:valid))
    get :edit, xhr: true, params: { topic_id: @comment.topic, id: @comment, format: 'js' }
    assert_not_nil assigns(:topic)
    assert_not_nil assigns(:comment)
  ...
  test 'should not get edit as banned user' do
    ...
    get :edit, xhr: true, params: {
      topic_id: comments(:banned).topic, id: comments(:banned)
    }, format: 'js'
    assert_not_nil assigns(:topic)
    assert_nil assigns(:comment)
    assert_response :success
  ...
  test 'should not update comment on locked topic' do
    login(users(:valid))
    patch :update, params: {
      topic_id: topics(:locked),
      id: comments(:three),
      comment: { description: 'Updated Description on Locked' }
    }
    assert_nil assigns(:topic)
    assert_nil assigns(:comment)
    assert_redirected_to topics_path
  ...
  test 'should not update comment as banned user' do
    ...
    patch :update, params: {
      topic_id: comments(:banned).topic, id: comments(:banned),
      comment: { description: 'I was banned so I am changing my comment' }
    }
    assert_not_nil assigns(:topic)
    assert_nil assigns(:comment)
    assert_redirected_to assigns(:topic)
